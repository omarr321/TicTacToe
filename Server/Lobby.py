...
class Lobby():
    ...
    def addPlayer(self, player, gridSize):
        if gridSize == 3:
            self.playerList3x3.append(player)
        elif gridSize == 4:
            self.playerList4x4.append(player)
        elif gridSize == 5:
            self.playerList5x5.append(player)
        else:
            raise ValueError ("Error: grid size is not 3,4, or 5!")

    def removePlayer(self, player, gridSize):
        if gridSize == 3:
            self.playerList3x3.remove(player)
        elif gridSize == 4:
            self.playerList4x4.remove(player)
        elif gridSize == 5:
            self.playerList5x5.remove(player)
        else:
            raise ValueError ("Error: grid size is not 3,4, or 5!")
    ...
    def run(self):
        while(True):
            if(self.__checkLobby(3)):
                newGame = GameServer.GameServer(self.playerList3x3.pop(), self.playerList3x3.pop(), 3)
            ...
            if(self.__checkLobby(4)):
                newGame = GameServer.GameServer(self.playerList4x4.pop(), self.playerList4x4.pop(), 4)
                game = threading.Thread(target=newGame.run)
                game.start()
                print("Done!")
            else:
            ...
            if(self.__checkLobby(5)):
                newGame = GameServer.GameServer(self.playerList5x5.pop(), self.playerList5x5.pop(), 5)
                game = threading.Thread(target=newGame.run)
                game.start()
                print("Done!")
            else:
